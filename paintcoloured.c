...
static Window create_win(int x, int y, int w, int h, int b)
{
    ...
    win = XCreateWindow(dpy, root, x, y, w, h, b, DefaultDepth(dpy, scr), InputOutput, vis, CWBackPixel | CWEventMask | CWBorderPixel, &xwa);
...
static GC create_gc(int line_width, XftColor *fg)
{
    ...
    xgcv.foreground = fg->pixel; /* This set foreground to blue*/
