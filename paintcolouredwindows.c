...
// static void create_color(Clr *color_blue, const char* name)
// {
...
static GC create_gc(int line_width)
{
    ...
    xgcv.foreground = BlackPixel(dpy, scr);  /* This set foreground to blue*/
...
int main()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        color_buttons[i].color = (Clr*)malloc(sizeof(Clr));
    ...
    XFreeGC(dpy, gc);
    // ClrFree(dpy, vis, DefaultColormap(dpy, scr), color_blue);
    for (int i = 0; i < 3; i++)
